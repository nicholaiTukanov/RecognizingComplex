...
void ComplexMulKernelOp::build(mlir::OpBuilder &builder, mlir::OperationState &state,
                  mlir::Value lhs, mlir::Value rhs, mlir::Value lhs2, mlir::Value rhs2 ) {
  state.addTypes(UnrankedTensorType::get(builder.getF64Type()));
  state.addOperands({lhs, rhs, lhs2, rhs2});
}
...
mlir::ParseResult ComplexMulKernelOp::parse(mlir::OpAsmParser &parser,
                               mlir::OperationState &result) {
  return parseBinaryOp(parser, result);
}
void ComplexMulKernelOp::print(mlir::OpAsmPrinter &p) { printBinaryOp(p, *this); }
...
void ComplexMulKernelOp::inferShapes() { getResult().setType(getOperand(0).getType()); }
